...
def generate_prime(): # gera o número primo - p e q
    ...
    while True: # 2**2048 são as chaves padrão do RSA
...
while (True):
    ...
    while len(text) > 128:
        ...
        text = input('Frase Inválida, Digite novamente: ')
    ...
    public_key_one = int(n)
    public_key_two = int(e)

    text_cifra = cifra(text,e,n)


    print('- Sua mensagem criptografada:', ''.join(list(map(chr,text_cifra))))

    d = calculate_private_key(totient_de_N,e)

    print('- Sua chave privada é:', d)
    print('- Sua primeira chave pública:', public_key_one)
    print('- Sua segunda chave pública:', public_key_two)
    ...
    while (p != d or decrip != ''.join(list(map(chr,text_cifra)))):

        print('Mensagem criptografada  chave privada inválidas, digite novamente!!')
    ...
    public_key_users_one = int(input("Digite a primeira chave publica : "))

    while (public_key_users_one != public_key_one):

        print('=-='*20)
        print('Primeira chave publica inválida, digite novamente!!')
        print('=-='*20)

        public_key_users_one = int(input("Digite a chave publica: "))


    public_key_users_two = int(input("Digite a segunda chave publica : "))

    while (public_key_users_two != public_key_two):

        print('=-='*20)
        print('Segunda chave publica inválida, digite novamente!!')
        print('=-='*20)
